...
const resolvers = {
  ...
  Listing: {
    host: ({ hostId }, _, { dataSources }) => {
      /**
       * Even when we return with that datasource
       * it still works. I guess as long as it returns
       * an id it should be fine. But in the case that you
       * don't have access to the datasource returning
       * just the id field is fine
       */
      // return dataSources.accountsAPI.getUser(hostId);
      return {
        id: hostId, // has to return the id field or else it won't work
      };
    },
    overallRating: ({ id }, _, { dataSources }) => {
  ...
  Review: {
    author: ({ authorId, targetType }, _, { dataSources }) => {
      ...
      return {
        id: authorId,
        // need to turn a typename here because author returns a User interface type
        // in which an interface type can't have a __resolveReference method
        __typename: ['LISTING', 'HOST'].includes(targetType) ? 'Guest' : 'Host',
      };
    },
  },
  AmenityCategory: {
